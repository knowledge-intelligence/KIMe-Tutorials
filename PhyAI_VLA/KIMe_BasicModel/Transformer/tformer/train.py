...
def get_ds(config):
    ...
    if config['datasource'] == 'lemon-mint/korean_english_parallel_wiki_augmented_v1':
        from dataset import TranslationDatasetForLemonmint as TranslationDataset
        # REF : https://huggingface.co/docs/datasets/v1.11.0/splits.html
        ...
        from dataset import TranslationDatasetForOpusbooks as TranslationDataset        
    ...
    train_ds = TranslationDataset(train_ds_raw, tokenizer_src, tokenizer_tgt, lang_src, lang_tgt, config['seq'])
    val_ds = TranslationDataset(val_ds_raw, tokenizer_src, tokenizer_tgt, lang_src, lang_tgt, config['seq'])
    ...
    for item in tqdm(ds_raw, desc=f"Processing Max Length(encode)"):
        if config['datasource'] == 'lemon-mint/korean_english_parallel_wiki_augmented_v1':
