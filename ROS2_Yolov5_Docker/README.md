# ROS2 Yolov5 Docker Tutorial (Humble Tested)

## Modify ROS Domain ID
```shell
echo $ROS_DOMAIN_ID
# or
export ROS_DOMAIN_ID=1 #(0 ~ 101)
```

## Check Webcam Index
### (If needed)
```shell
sudo apt install v4l-utils
```
```shell
v4l2-ctl --list-devices
```

## Modify the Webcam index in webcam_pub.py
- Python 코드에서 변경
```python
#(Line 38 in webcam_pub.py)
self.cap = cv2.VideoCapture(0)
```
- ROS Run 파라미터 변경
```bash
# 기본 실행 (0번 카메라)
ros2 run ros2_yolov5 webcam_pub

#특정 인덱스 카메라: 
ros2 run ros2_yolov5 webcam_pub --ros-args -p source:=1
```


## Build Dockerfile
- GPU
```shell
cd ~/yolo_ws/ROS2_Yolov5_Docker
sudo docker build -f Dockerfile . -t yolov5
```
- CPU
```shell
cd ~/yolo_ws/ROS2_Yolov5_Docker
sudo docker build -f Dockerfile_CPU . -t yolov5_cpu
```


## Run Docker Image
```shell
sudo docker run --rm -it --privileged \
   --net=host -e DISPLAY=$DISPLAY \
   -v /tmp/.X11-unix:/tmp/.X11-unix  \
   -v /tmp/runtime-user:/tmp/runtime-user \
   --name yolov5-docker yolov5
```

## 호스트에서 먼저 X11 접근 허용 (컨테이너 root가 디스플레이에 그리도록)
```shell
xhost +local:docker

sudo docker run --rm -it \
  --net=host \
  --ipc=host \
  --gpus all \
  -e DISPLAY=$DISPLAY \
  -e ROS_DOMAIN_ID=0 \
  -e RMW_IMPLEMENTATION=rmw_fastrtps_cpp \
  -v /tmp/.X11-unix:/tmp/.X11-unix:rw \
  -v /tmp/runtime-user:/tmp/runtime-user \
  -v ~/yolo_ws:/root/yolo_ws \
  --name yolov5-docker \
  yolov5
```



## Build ROS2 nodes for Out_Docker
```shell
source /opt/ros/jazzy/setup.bash
cd ~/yolo_ws/ROS2_Yolov5_Docker
colcon build --symlink-install --packages-select ros2_yolov5
```

## Run Out_Docker Nodes - Publisher
```shell
source /opt/ros/jazzy/setup.bash
source ~/yolo_ws/ROS2_Yolov5_Docker/install/setup.bash
export ROS_DOMAIN_ID=1
ros2 run ros2_yolov5 img_publisher
```

```shell
# 기본 실행 (0번 카메라): 
ros2 run ros2_yolov5 webcam_pub

# 특정 인덱스 카메라: 
ros2 run ros2_yolov5 webcam_pub --ros-args -p source:=1

# 동영상 파일: 
ros2 run ros2_yolov5 webcam_pub --ros-args -p source:="/path/to/video.mp4"
```

## Run Out_Docker Nodes - Subscriber
```shell
source /opt/ros/jazzy/setup.bash
source ~/yolo_ws/ROS2_Yolov5_Docker/install/setup.bash
export ROS_DOMAIN_ID=1
ros2 run ros2_yolov5 img_subscriber
```





# Docker 관련 CMD

## Docker Images/ps list
```shell
$ sudo docker images list
$ sudo docker ps
```

## GPU 접근이 실제로 되는지 확인
```bash
docker exec -it yolov5-docker nvidia-smi
```

## rm 옵션, 일회성으로 실행
```shell
$ docker run --rm -it --entrypoint bash \<image-name-or-id\>
```

## to enter a running container
(-it 컨테이너를 종료하지 않고, 터미널의 입력을 계속해서 컨테이너로 전달하기 위해서 사용)
```shell
docker exec -it \<container-name-or-id\> bash
```